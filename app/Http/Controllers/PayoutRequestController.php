...
class PayoutRequestController extends Controller
{
  ...
  public function payout($id = null)
  {
    ...
    if ($id != null || $id != '') {
      $withdrawal = DB::table('withdrawals')->join('conducteur', 'conducteur.id', '=', 'withdrawals.id_conducteur')
        ->select('withdrawals.*', 'conducteur.nom', 'conducteur.prenom')
        ->where('withdrawals.id_conducteur', '=', $id)->orderBy('id', 'desc');
    } else {
      $withdrawal = DB::table('withdrawals')->join('conducteur', 'conducteur.id', '=', 'withdrawals.id_conducteur')
        ->select('withdrawals.*', 'conducteur.nom', 'conducteur.prenom')->where('withdrawals.statut', '=', 'pending')->orderBy('id', 'desc');
    }
    $totalLength = count($withdrawal->get());
    $withdrawal = $withdrawal->paginate(20);
    return view("payout_request.index", compact('withdrawal', 'currency', 'totalLength'));
  }

  public function getBankDetails(Request $request)
  {
    $id = $request->input('id');
    $bankDetails = DB::table('conducteur')->select('*')->where('id', '=', $id)->get();
  ...
  public function acceptWithdrawal(Request $request)
  {
    ...
    $driver = DB::table('conducteur')->select('amount', 'email', 'nom', 'prenom')->where('id', '=', $driver_id)->first();
    $newDriverAmount = $driver->amount - $withdraw_amount;
    DB::table('conducteur')->where('id', '=', $driver_id)->update(['amount' => $newDriverAmount]);
    if ($withdrawal) {
