...
class ReportController extends Controller
{
    ...
    public function downloadExcel(Request $request){
        $request->validate([
            ...
            'type.required' => trans('lang.please_select_file_type_before_downloading'),            
        ]); 
        $finalarr=array();         
        ...
        if($users->count() > 0){
            ...
            foreach ($users as $row1 => $k1){
                ...
                $followupslist = DB::table('user_app as user')->select('user.*')->get();
            ...
            if($type== 'csv'){
                ...
                foreach($users as $row){
                    ...
                    $pdf->Cell(40,7,Helper::shortEmail($row['email']),1,0);
                    $pdf->Cell(40,7,Helper::shortNumber($row['phone']),1,0);
           ...
           foreach ($users as $row){
                ...
                $listarray=array('user.id','user.nom','user.prenom','user.email','user.phone','user.login_type','user.photo','user.photo_path','user.photo_nic','user.photo_nic_path','user.statut','user.statut_nic','user.tonotify','user.device_id','user.fcm_id','user.creer','user.updated_at','user.modifier','user.amount','user.reset_password_otp','user.reset_password_otp_modifier','user.age','user.gender','user.created_at');

                $list = DB::table('user_app as user')
                        ->select($listarray)
...

                {
                    ...
                    $row5['email']=Helper::shortEmail($row['email']);

                    $row5['phone']=Helper::shortNumber($row['phone']);
            ...
            $error = trans('lang.no_data_found');
    ...
    public function downloadExcelDriver(Request $request){   
        ...
        $drivers11 = array("Sr. No.","First Name", "Last Name","Phone", "Latitude", "Longitude","email","Status","online", "Login Type", "Photo Path","Tonotify","Address", "Bank Name", "Branch Name", "Holder Name" ,"Account No","Other Info","Creer","Modifier","Amount"); 
            ...
            foreach ($drivers as $row1 => $k1) {
                 ...
                 $followupslist = DB::table('conducteur as driver')->select('driver.*')->get();
...

            {
                ...
                foreach($drivers as $row)
                {
                    ...
                    $pdf->Cell(30,7,Helper::shortNumber($row['phone']),1,0);
            ...
            foreach ($drivers as $row) {
                ...
                $list = DB::table('conducteur as driver')
                        ->select($listarray)
    ...
    {
        return view("reports.ownerreport");
    }
    public function downloadExcelOwner(Request $request)
    {
        $finalarr = array();
        $status = $request->input('user_status');
        if ($request->input('date') == 'today') {
        ...
        if ($drivers->count() > 0) {
            $drivers11 = array("Sr. No.", "First Name", "Last Name", "Phone","email", "Status","Login Type", "Creer", "Amount");
            $temp_max = 0;
            ...
            $filename = "Owner_report_" . date("Y.m.d") . "." . $type;
            ...
            foreach ($drivers as $row) {
                ...
                $listarray = array('driver.id', 'driver.nom', 'driver.prenom', 'driver.phone', 'driver.email', 'driver.statut', 'driver.login_type', 'driver.creer', 'driver.amount');
                $list = DB::table('conducteur as driver')
                    ->select($listarray)
                    ->where('driver.id', $row['id'])
                    ->get();
    ...
    public function downloadExcelTravel(Request $request){       
        ...
        $rides11 = array("S.No","Trip Start From","Trip End To","User Name","Driver Name","Trip Status","Paid Status","Payment Option","Trip Distance","Total Amount"); 
            ...
            foreach ($rides as $row1 => $k1) {
                 ...
                 $followupslist=array();
                 $followupslist = DB::table('requete as request')
                 ->join('user_app', 'request.id_user_app', '=', 'user_app.id')
                 ->join('conducteur', 'request.id_conducteur', '=', 'conducteur.id')
                 ->join('payment_method', 'request.id_payment_method', '=', 'payment_method.id')
                 ->select('request.id','request.statut', 'request.statut_paiement','request.depart_name','request.destination_name','request.distance','request.montant','request.creer','conducteur.id as driver_id','conducteur.prenom as driverPrenom','conducteur.nom as driverNom','user_app.id as user_id','user_app.prenom as userPrenom','user_app.nom as userNom','payment_method.libelle','payment_method.image')
                 ->where('conducteur.id','=','request.id_conducteur')
                 ->where('user_app.id','=','request.id_user_app')
                 ->where('payment_method.id','=','request.id_payment_method')
                 ->get();
                $finalarr=$followupslist->toArray();
           ...
           for ($i=0; $i < $temp_max; $i++)
            ...
            foreach ($rides as $row) {
                 ...
                 $listarray=array('request.id','request.depart_name','request.destination_name','user_app.nom','conducteur.nom','request.statut','request.statut_paiement','payment_method.libelle','request.distance','request.montant');
                 
                 $list = DB::table('requete as request')
                ->join('user_app', 'request.id_user_app', '=', 'user_app.id')
                 ->join('conducteur', 'request.id_conducteur', '=', 'conducteur.id')
                 ->join('payment_method', 'request.id_payment_method', '=', 'payment_method.id')
                 ->select('request.id','request.statut', 'request.statut_paiement','request.depart_name','request.destination_name','request.distance','request.montant','request.creer','conducteur.id as driver_id','conducteur.prenom as driverPrenom','conducteur.nom as driverNom','user_app.id as user_id','user_app.prenom as userPrenom','user_app.nom as userNom','payment_method.libelle','payment_method.image')
                 ->where('conducteur.id',$row['id_conducteur'])   
                 ->where('user_app.id',$row['id_user_app'])   
                 ->where('payment_method.id',$row['id_payment_method'])  
                 ->get();
