...
class UpdateFcmController extends Controller
{
    ...
    public function updatefcm(Request $request)
    {
        ...
        if ($user_cat == "user_app") {
            $update_query = DB::update('update user_app set fcm_id = ?,device_id = ?,modifier = ? where id = ?', [$fcm_id, $device_id, $date_heure, $user_id]);
            if ($update_query) {
            ...
            $update_query = DB::update('update conducteur set fcm_id = ?,device_id = ?,modifier = ? where id = ?', [$fcm_id, $device_id, $date_heure, $user_id]);
            if ($update_query) {
    ...
    public function getServiceJson(){
        ...
        if (!File::exists($path)) {
            return response()->json(['error' => 'Firebase credentials not found'], 404);
        }
