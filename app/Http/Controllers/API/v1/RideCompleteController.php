...
class RideCompleteController extends Controller
{
    public function completeRequest(Request $request)
    {
        ...
        $rideData = Requests::find($id_ride);
        if(in_array($rideData->statut,['completed'])){
            $response['success'] = 'failed';
            $response['code'] = 404;
        ...
        try {
            ...
            //Get subtotal
            $sub_total = $rideData->montant;
            $totalAmount = $sub_total;

            //Calculate discount
            $discount_type = json_decode($rideData->discount_type, true);
            if($discount_type){
            ...
            $commission_amount = 0;
            if (!empty($admin_commisions) && $rideData->admin_commission_type != null) {
                $adminCommission = json_decode($rideData->admin_commission_type, true);
            ...
            $totalTaxAmount = 0;
            $taxHtml = '';
            $tax = json_decode($rideData->tax, true);
            if (!empty($tax)) {
            ...
            //Check tip
            if (!empty($tip) && $tip > 0) {
                $totalAmount = $totalAmount + floatval($tip);
            }
            ...
            //Update ride data
            $rideData->statut_paiement = "yes";
            $rideData->id_payment_method = $id_payment;
            $rideData->tip_amount = $tip;
            $rideData->discount = $discount;
            $rideData->transaction_id = $transaction_id;
            $rideData->admin_commission = $commission_amount;
            ...
            if ($referral && $referral->referral_by_id) {
                ...
                Transaction::create([
                    ...
                    'transaction_id' => 'REF_'.$id_ride.'_'.$id_user,
                ]);
            ...
            //Send notification
            $message = array("body" => "Your customer has just paid for his ride", "title" => "Payment of the ride", "sound" => "mySound", "tag" => "ridecompleted");
            $fcm_token = $driverData->fcm_id;
            if (!empty($fcm_token)) {
                GcmController::sendNotification($fcm_token, $message);
                Notification::create([
                    'titre' => 'End of your ride',
                    'message' => $driverData->prenom." " .$driverData->nom . " is completed your ride.",
                    'statut' => 'yes',
            ...
            if ($userData->email != "") {
                $emailsubject = '';
                $emailmessage = '';
                $emailtemplate = EmailTemplate::where('type', 'payment_receipt')->first();
                ...
                $emailmessage = str_replace("{Distance}", number_format($rideData->distance, 2) . " " . $rideData->distance_unit, $emailmessage);
                $emailmessage = str_replace("{Duree}", $rideData->duree, $emailmessage);
            ...
            Log::error('Complete Requete API: Failed: ' . $e->getMessage());
