...
class AssignDriverJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($this->booking->statut !== 'new' || $this->booking->id_conducteur) {
            return;
        }
        ...
        $lat = $this->booking->latitude_depart;
        $long = $this->booking->longitude_depart;
        $vehicle_type_id = $this->booking->vehicle_type_id;
        
        $userZoneId = Helper::getUserZoneId($lat, $long);
        if (empty($userZoneId)) {
            //No drivers left — cancel the ride
            $this->booking->statut = 'canceled';
            $this->booking->save();
            Log::warning('No driver found. Booking cancelled.', ['booking_id' => $this->booking->id]);
            return;
        }
        ...
        if ($newDriver) {
            ...
            if(Helper::isDriverBookingAllowed($newDriver->id,'subscriptionTotalOrders')){
                ...
                AssignDriverJob::dispatch($this->booking)->delay(now()->addSeconds($accept_reject_time));

                Log::info('AssignDriverJob triggered from job', ['booking_id' => $this->booking->id, 'driver_id' => $newDriver->id]);
                ...
                Log::info('AssignDriverJob triggered from job and fetched driver is not able to get order due to limit', 
