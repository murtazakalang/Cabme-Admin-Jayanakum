                            ...
                            <div class="card-header d-flex justify-content-between align-items-center border-0">
                                ...
                                <div class="card-header-right d-flex align-items-center">
                                    <div class="card-header-btn mr-3">
                                        <a class="btn-primary btn rounded-full" href="{!! route('vehicle-type.create') !!}"><i class="mdi mdi-plus mr-2"></i>{{ trans('lang.create_vehicle') }}</a>
                                    </div>
                                </div>
                            </div>
                            <div class="card-body">
                                    ...
                                    <div id="users-table_filter" class="ml-auto">
                                        <label>{{ trans('lang.search_by') }}
                                            <div class="form-group">
                                                <form action="{{ route('vehicle-type.index') }}" method="get">
                                                    ...
                                                    <div class="search-box position-relative">
                                                        ...
                                                        <a class="btn btn-warning btn-flat" href="{{ route('vehicle-type.index') }}">
                                                            {{ trans('lang.clear') }}
                                                        </a>
                                                    </div>
                                                </form>
                                ...
                                <div class="table-responsive m-t-10">
                                        ...
                                        <tbody id="append_list12">
                                            @if (count($types) > 0)
                                                @foreach ($types as $type)
                                                    <tr>
                                                        @can('vehicle-type.delete')
                                                        <td class="delete-all"><input type="checkbox" id="is_open_{{ $type->id }}" class="is_open" dataid="{{ $type->id }}"><label class="col-3 control-label" for="is_open_{{ $type->id }}"></label></td>
                                                        @endcan
                                                        @if (file_exists(public_path('assets/images/type_vehicle' . '/' . $type->image)) && !empty($type->image))
                                                            <td><img class="rounded" style="width:50px" src="{{ asset('assets/images/type_vehicle') . '/' . $type->image }}" alt="image">
                                                            <a href="{{ route('vehicle-type.edit', ['id' => $type->id]) }}">{{ $type->libelle }}</a>
                                                            </td>
                                                        @else
                                                        ...
                                                        <td>
                                                            ...
                                                            @else
                                                                <label class="switch"><input type="checkbox" id="{{ $type->id }}" name="isActive"><span class="slider round"></span></label>
                                                            @endif
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td class="action-btn"><a href="{{ route('vehicle-type.edit', ['id' => $type->id]) }}" data-toggle="tooltip" data-bs-original-title="{{ trans('lang.edit') }}"><i class="mdi mdi-lead-pencil"></i></a>
                                                        @can('vehicle-type.delete')
                                                            <a id="'+val.id+'" class="delete-btn" href="{{ route('vehicle-type.delete', ['id' => $type->id]) }}" data-toggle="tooltip" data-bs-original-title="{{ trans('lang.delete') }}"><i class="mdi mdi-delete"></i></a>
                                                        @endcan
                                                        </td>
                                                    </tr>
                                                @endforeach
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        <div>
                                            {{trans('lang.showing')}} {{ $types->firstItem() }} {{trans('lang.to_small')}} {{ $types->lastItem() }} {{trans('lang.of')}} {{ $types->total() }} {{trans('lang.entries')}}
                                        </div>
                                        <div>
