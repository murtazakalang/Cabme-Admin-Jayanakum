    ...
    <div class="page-wrapper">
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h3 class="text-themecolor">{{ trans('lang.zones') }}</h3>
            </div>
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ url('/dashboard') }}">{{ trans('lang.dashboard') }}</a></li>
                    <li class="breadcrumb-item active">{{ trans('lang.zones') }}</li>
                </ol>
            </div>
            <div>
            </div>
                            ...
                            <div class="card-body">
                                <div class="userlist-topsearch d-flex mb-3">
                                    ...
                                    <div id="users-table_filter" class="ml-auto">
                                        <label>{{ trans('lang.search_by') }}
                                            <div class="form-group mb-0">
                                                <form action="{{ route('zone.index') }}" method="get">
                                                    ...
                                                    <div class="search-box position-relative">
                                                        ...
                                                        <a class="btn btn-warning btn-flat" href="{{ url('zone') }}">
                                                            {{ trans('lang.clear') }}
                                                        </a>
                                                    </div>
                                                </form>
                                ...
                                <div class="table-responsive m-t-10">
                                        ...
                                        <tbody id="append_list1">
                                            @if (count($zones) > 0)
                                                @foreach ($zones as $zone)
                                                    <tr>
                                                        @can('zone.delete')
                                                        <td class="delete-all"><input type="checkbox" id="is_open_{{ $zone->id }}" class="is_open" dataid="{{ $zone->id }}"><label class="col-3 control-label" for="is_open_{{ $zone->id }}"></label></td>
                                                        @endcan
                                                        <td><a href="{{ route('zone.edit', ['id' => $zone->id]) }}">{{ $zone->name }}</a>
                                                        </td>
                                                        <td>
                                                            @if ($zone->status == 'yes')
                                                                <label class="switch"><input type="checkbox" id="{{ $zone->id }}" name="isSwitch" checked><span class="slider round"></span></label>
                                                            @else
                                                                <label class="switch"><input type="checkbox" id="{{ $zone->id }}" name="isSwitch"><span class="slider round"></span></label>
                                                            @endif
                                                        </td>
                                                        <td>
                                                            <span class="date">{{ date('d F Y', strtotime($zone->created_at)) }}</span>
                                                            <span class="time">{{ date('h:i A', strtotime($zone->created_at)) }}</span>
                                                        </td>
                                                        <td>
                                                            <span class="date">{{ date('d F Y', strtotime($zone->updated_at)) }}</span>
                                                            <span class="time">{{ date('h:i A', strtotime($zone->updated_at)) }}</span>
                                                        </td>
                                                        <td class="action-btn"><a href="{{ route('zone.edit', ['id' => $zone->id]) }}" class="do_not_edit" data-toggle="tooltip" data-bs-original-title="{{ trans('lang.edit') }}"><i class="mdi mdi-lead-pencil"></i></a>
                                                        @can('zone.delete')
                                                            <a href="{{ route('zone.delete', ['id' => $zone->id]) }}" class="delete-btn" data-toggle="tooltip" data-bs-original-title="{{ trans('lang.delete') }}"><i class="mdi mdi-delete"></i></a>
                                                        @endcan
                                                        </td>
                                                    </tr>
                                                @endforeach
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        <div>
                                            {{trans('lang.showing')}} {{ $zones->firstItem() }} {{trans('lang.to_small')}} {{ $zones->lastItem() }} {{trans('lang.of')}} {{ $zones->total() }} {{trans('lang.entries')}}
                                        </div>
                                        <div>
