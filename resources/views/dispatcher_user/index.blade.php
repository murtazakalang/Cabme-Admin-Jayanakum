                ...
                <div class="col-12">
                    <div class="d-flex top-title-section pb-4 justify-content-between">
                       <div class="d-flex top-title-left align-self-center">
                            ...
                            <h3 class="mb-0">{{ trans('lang.dispatcher_user') }}</h3>
                            <span class="counter ml-3 zone_count">{{$totalLength}}</span>
                        </div>
                        <form action="{{ route('dispatcher-users.index') }}" method="get" id="filterForm">
                ...
                <div class="card border">
                        <div class="card-header d-flex justify-content-between align-items-center border-0">
                                <div class="card-header-title">
                                    <h3 class="text-dark-2 mb-2 h4">{{ trans('lang.dispatcher_user') }}</h3>
                                    <p class="mb-0 text-dark-2">{{ trans('lang.dispatcher_user_text') }}</p>
                                </div>
                                <div class="card-header-right d-flex align-items-center">
                                    <div class="card-header-btn mr-3">
                                        <a class="btn-primary btn rounded-full" href="{!! route('dispatcher-users.create') !!}"><i class="mdi mdi-plus mr-2"></i>{{ trans('lang.dispatcher_user_create') }}</a>
                                    </div>
                                </div>
                            </div>
                    <div class="card-body">
                        <div class="userlist-topsearch-flex mb-3">  
                            ...
                            <div class="userlist-topsearch d-flex mb-0">
                                <div id="users-table_filter" class="ml-auto">
                                    <label>{{ trans('lang.search_by')}}
                                        <div class="form-group mb-0">
                                            <form action="{{ route('dispatcher-users.index') }}" method="get">
                                                ...
                                                <div class="search-box position-relative">
                                                    ...
                                                    <a class="btn btn-warning btn-flat" href="{{ url('dispatcher-users') }}">
                                                        {{ trans('lang.clear') }}
                                                    </a>
                                                </div>
                                            </form>
                            ...
                            <div class="dropdown text-right">
                                ...
                                <ul class="dropdown-menu" aria-labelledby="exportDropdown">
                                    <li><a class="dropdown-item" href="{{ route('export.data', ['type' => 'excel', 'model' => 'DispatcherUser']) }}">{{ trans("lang.export_excel") }}</a></li>
                                    <li><a class="dropdown-item" href="{{ route('export.data', ['type' => 'pdf', 'model' => 'DispatcherUser']) }}">{{ trans("lang.export_pdf") }}</a></li>
                                    <li><a class="dropdown-item" href="{{ route('export.data', ['type' => 'csv', 'model' => 'DispatcherUser']) }}">{{ trans("lang.export_csv") }}</a></li>
                                </ul>
                            </div>
                        </div>
                            ...
                            <table id="example24" class="display nowrap table table-hover table-striped table-bordered table table-striped" cellspacing="0" width="100%">
                                ...
                                <tbody id="append_list12">
                                    ...
                                    <tr>
                                        ...
                                        @endcan
                                        @php
                                            $relativePath = str_replace(url('/') . '/', '', $customer->profile_picture_path);
                                            $imagePath = !empty($customer->profile_picture_path) && file_exists(public_path($relativePath)) 
                                                        ? $customer->profile_picture_path 
                                                        : asset('assets/images/placeholder_image.jpg');
                                        @endphp
                                        <td>
                                            <img class="rounded" style="width:50px" src="{{ $imagePath }}" alt="image">
                                            <a href="{{ route('dispatcher-users.show', ['id' => $customer->id]) }}">
                                        ...
                                        @endif
                                        <td class="action-btn">
                                            <a href="{{route('dispatcher-users.show', ['id' => $customer->id])}}" class="" data-toggle="tooltip" data-bs-original-title="{{ trans('lang.view_details') }}"><i class="mdi mdi-eye"></i></a>
                                            <a href="{{route('dispatcher-users.edit', ['id' => $customer->id])}}" data-toggle="tooltip" data-bs-original-title="{{ trans('lang.edit') }}"><i class="mdi mdi-lead-pencil"></i></a>
                                            @can('dispatcher-users.delete')
                                            <a id="'+val.id+'" class="delete-btn" name="user-delete" href="{{route('dispatcher-users.delete', ['id' => $customer->id])}}" data-toggle="tooltip" data-bs-original-title="{{ trans('lang.delete') }}"><i class="mdi mdi-delete"></i></a>
                                        @endcan
                                        </td>
                                    </tr>
                                    @endforeach
...
<script type="text/javascript">
    ...
    $(document).on("click", "input[name='isActive']", function(e) {
        ...
        $.ajax({
            ...
            url: '{{url("dispatcher-users-switch")}}',
            method: "POST",
            data: {
                'ischeck': ischeck,
                'id': id
