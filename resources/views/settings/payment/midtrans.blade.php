                        ...
                        <div class="card-body">
                            ...
                            <div class="row restaurant_payout_create">
                                <div class="restaurant_payout_create-inner">
                                    <fieldset>
                                        @foreach($midtrans as $data)
                                            <legend>{{trans('lang.midtrans')}}
                                            </legend>
                                            <div class="form-check width-100">
                                                <input type="hidden" class="id" id="id" value="{{$data->id}}">
                                                <input type="checkbox" class="enable_midtrans" value="{{$data->isEnabled}}"
                                                    id="enable_midtrans" @if($data->isEnabled == "true") checked @endif>
                                                <label class="col-3 control-label"
                                                    for="enable_midtrans">{{trans('lang.app_setting_enable_midtrans')}}</label>
                                            </div>
                                            <div class="form-check width-100">
                                                <input type="checkbox" class="enable_midtrans_sandbox"
                                                    value="{{$data->isSandboxEnabled}}" id="enable_midtrans_sandbox"
                                                    @if($data->isSandboxEnabled == "true") checked @endif>
                                                <label class="col-3 control-label"
                                                    for="enable_midtrans_sandbox">{{trans('lang.app_setting_enable_midtrans_sandbox')}}</label>
                                            </div>
                                            <div class="form-group row width-100">
                                                <label
                                                    class="col-3 control-label">{{trans('lang.app_setting_midtrans_server_key')}}</label>
                                                <div class="col-7">
                                                    <input type="password" class="form-control server_key"
                                                        value="{{$data->key}}">
                                                    <div class="form-text text-muted">
                                                        {!! trans('lang.app_setting_midtrans_server_key_help') !!}
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach                                
...
<script>
    ...
    $(".edit-setting-btn").click(function () {
        ...
        var enableSandbox = $(".enable_midtrans_sandbox").is(":checked");
        var server_key = $('.server_key').val();
        var id = $('.id').val();
        var url = "{{ route('payment.midtransUpdate', ':id') }}";
