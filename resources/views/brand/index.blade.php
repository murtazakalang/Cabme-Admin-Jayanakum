                        ...
                        <div class="card border">
                            <div class="card-header d-flex justify-content-between align-items-center border-0">
                                <div class="card-header-title">
                                    <h3 class="text-dark-2 mb-2 h4">{{ trans('lang.brand') }}</h3>
                                    <p class="mb-0 text-dark-2">{{ trans('lang.brand_table_text') }}</p>
                                </div>
                                <div class="card-header-right d-flex align-items-center">
                                    <div class="card-header-btn mr-3">
                                        <a class="btn-primary btn rounded-full" href="{!! route('brand.create') !!}"><i class="mdi mdi-plus mr-2"></i>{{ trans('lang.create_brand') }}</a>
                                    </div>
                                </div>
                            </div>
                            <div class="card-body">
                                                ...
                                                <div class="form-group mb-0">
                                                    <form action="{{ route('brand.index') }}" method="get">
                                                        ...
                                                        <div class="search-box position-relative">
                                                            ...
                                                            <a class="btn btn-warning btn-flat" href="{{ url('brands') }}">{{ trans('lang.clear') }}</a>
                                                        </div>
                                                    </form>
                                                </div>
                                            </label>
                                ...
                                <div class="table-responsive m-t-10">
                                        ...
                                        <tbody id="append_list1">
                                            @if (count($brands) > 0)
                                                @foreach ($brands as $brand)
                                                    <tr>
                                                        @can('brand.delete')
                                                        <td class="delete-all"><input type="checkbox" id="is_open_{{ $brand->id }}" class="is_open" dataid="{{ $brand->id }}"><label class="col-3 control-label" for="is_open_{{ $brand->id }}"></label></td>
                                                        @endcan
                                                        <td><a href="{{ route('brand.edit', ['id' => $brand->id]) }}">{{ $brand->name }}</a>
                                                        </td>
                                                        <td>
                                                            @if ($brand->status == 'yes')
                                                                <label class="switch"><input type="checkbox" id="{{ $brand->id }}" name="isActive" checked><span class="slider round"></span></label>
                                                            @else
                                                                <label class="switch"><input type="checkbox" id="{{ $brand->id }}" name="isActive"><span class="slider round"></span></label>
                                                            @endif
                                                        </td>
                                                        <td>
                                                        ...
                                                        <td>
                                                            <span class="date">{{ date('d F Y', strtotime($brand->updated_at)) }}</span>
                                                            <span class="time">{{ date('h:i A', strtotime($brand->updated_at)) }}</span>
                                                        </td>
                                                        <td class="action-btn"><a href="{{ route('brand.edit', ['id' => $brand->id]) }}"><i class="mdi mdi-lead-pencil" data-toggle="tooltip" data-bs-original-title="{{ trans('lang.edit') }}"></i></a>
                                                            @can('brand.delete')
                                                            <a href="{{ route('brand.delete', ['id' => $brand->id]) }}" class="delete-btn"><i class="mdi mdi-delete" data-toggle="tooltip" data-bs-original-title="{{ trans('lang.delete') }}"></i></a>
                                                            @endcan
                                                        </td>
                                                    </tr>
                                                @endforeach
                                    ...
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        <div>
                                            {{trans('lang.showing')}} {{ $brands->firstItem() }} {{trans('lang.to_small')}} {{ $brands->lastItem() }} {{trans('lang.of')}} {{ $brands->total() }} {{trans('lang.entries')}}
                                        </div>
                                        <div>
