...
<div class="page-wrapper">
	<div class="row page-titles">
		<div class="col-md-5 align-self-center">
			<h3 class="text-themecolor">{{trans('lang.driver_document_edit')}}</h3>
		</div>
		...
		<div class="card pb-4">
				<div class="card-body">
					...
					<div class="error_top"></div>
						...
						@endif
					<form action="{{ route('driver-document.update',$document->id) }}" method="post"  enctype="multipart/form-data" id="create_driver">
					@csrf
					@method("PUT")
						<div class="row restaurant_payout_create">
							<div class="restaurant_payout_create-inner">
								<fieldset>
									<legend>{{trans('lang.driver_document_edit')}}</legend>
										<div class="form-group row width-100">
											<label class="col-3 control-label">{{trans('lang.title')}}</label>
											<div class="col-7">
												<input type="text" class="form-control libelle" name="title" value="{{$document->title}}">
										...
										<div class="form-group row width-100">
											...
											<div class="form-check width-100">
												<input type="radio" id="driver" name="type" value="driver" {{$document->type == 'driver' ? 'checked' : ''}}>
												<label class="control-label" for="driver">{{ trans('lang.individual_driver') }}</label>
											</div>
											<div class="form-check width-100">
												<input type="radio" id="owner" name="type" value="owner" {{$document->type == 'owner' ? 'checked' : ''}}>
												<label class="control-label" for="owner">{{ trans('lang.owner') }}</label>
											</div>
										</div>
										<div class="form-group row width-100">
											<div class="form-check">
												@if ($document->is_enabled === "Yes")
